...
function CheckoutContent() {
  ...
  const initializeCheckout = async () => {
    try {
      ...
      // Fetch car details, website info, and terms in parallel
      const [carResponse, websiteResponse, termsResponse] = await Promise.all([
  ...
  const getAvailablePaymentMethods = () => {
    if (!websiteInfo?.paymentSettings) return ['cash'];
    
    const methods = [];
    if (websiteInfo.paymentSettings.enablePaypal) methods.push('paypal');
    if (websiteInfo.paymentSettings.enableCreditCard) methods.push('credit_card');
    if (websiteInfo.paymentSettings.enableBankTransfer) methods.push('bank_transfer');
    if (websiteInfo.paymentSettings.enableCash) methods.push('cash');
